...
describe('OrderService', () => {
  ...
  it('gets a list of Orders and sets the correct query string', async () => {
    const pagedResult = {
      items: [
    ...
    const result = await firstValueFrom(sut.getOrders(filter));
  ...
  it('gets a list of Orders for customer and sets the correct query string', async () => {
    const pagedResult = {
      items: [
        { id: 1, customerPurchaseOrderNumber: 'CPON 1' },
        { id: 2, customerPurchaseOrderNumber: 'CPON 2' }
    ...
    const result = await firstValueFrom(sut.getOrders(filter, 1));
