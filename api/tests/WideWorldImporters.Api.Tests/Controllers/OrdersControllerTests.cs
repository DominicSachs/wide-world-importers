...
public sealed class OrdersControllerTests
{
	...
	public async Task Get_Returns_Paged_List_Result()
	{
		...
		_queryDispatcher.HandleAsync<GetOrdersQuery, PagedResult<OrderListResponse>>(Arg.Any<GetOrdersQuery>(), token).Returns(mockResult);

		var result = await _sut.Get(new(), null, token);
