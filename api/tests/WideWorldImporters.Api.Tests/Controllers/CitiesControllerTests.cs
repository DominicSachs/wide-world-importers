...
public sealed class CitiesControllerTests
{
	...
	public async Task Get_Returns_Paged_List_Result()
	{
		...
		_queryDispatcher.HandleAsync<GetCitiesQuery, PagedResult<CityListResponse>>(Arg.Any<GetCitiesQuery>(), token).Returns(mockResult);
	...
	public async Task GetById_Returns_A_City()
	{
		...
		_queryDispatcher.HandleAsync<GetCityQuery, CityEditResponse>(Arg.Is<GetCityQuery>(a => a.Id == 1), token).Returns(response);
	...
	public async Task Update_Returns_BadResult_If_Ids_Do_Not_Match()
	{
		...
		await _commandDispatcher.DidNotReceive().HandleAsync(Arg.Any<UpdateCityCommand>(), CancellationToken.None);
