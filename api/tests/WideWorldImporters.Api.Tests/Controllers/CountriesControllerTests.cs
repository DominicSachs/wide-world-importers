...
public sealed class CountriesControllerTests
{
	...
	public async Task Get_Returns_Paged_List_Result()
	{
		...
		_queryDispatcher.HandleAsync<GetCountriesQuery, PagedResult<CountryListResponse>>(Arg.Any<GetCountriesQuery>(), token).Returns(mockResult);
	...
	public async Task GetById_Returns_A_Country()
	{
		...
		_queryDispatcher.HandleAsync<GetCountryQuery, CountryEditResponse>(Arg.Is<GetCountryQuery>(a => a.Id == 1), token).Returns(response);
	...
	public async Task GetCountryStates_Returns_A_KeyValueList()
	{
		...
		_queryDispatcher.HandleAsync<GetCountryStateNamesQuery, List<KeyValuePair<int, string>>>(Arg.Is<GetCountryStateNamesQuery>(a => a.CountryId == 1), token).Returns(response);
	...
	public async Task GetNames_Returns_A_KeyValueList()
	{
		...
		_queryDispatcher.HandleAsync<GetCountryNamesQuery, List<KeyValuePair<int, string>>>(Arg.Any<GetCountryNamesQuery>(), token).Returns(response);
	...
	public async Task Update_Returns_BadResult_If_Ids_Do_Not_Match()
	{
		...
		await _commandDispatcher.DidNotReceive().HandleAsync(Arg.Any<UpdateCountryCommand>(), CancellationToken.None);
