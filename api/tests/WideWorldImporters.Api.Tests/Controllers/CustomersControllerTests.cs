...
public sealed class CustomersControllerTests
{
	...
	public async Task Get_Returns_Paged_List_Result()
	{
		...
		_queryDispatcher.HandleAsync<GetCustomersQuery, PagedResult<CustomerListResponse>>(Arg.Any<GetCustomersQuery>(), token).Returns(mockResult);
	...
	public async Task GetById_Returns_A_Customer()
	{
		...
		_queryDispatcher.HandleAsync<GetCustomerQuery, CustomerEditResponse>(Arg.Is<GetCustomerQuery>(a => a.Id == 1), token).Returns(response);
